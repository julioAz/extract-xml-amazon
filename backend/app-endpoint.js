...
app.post('/gerar-xml', async (req, res) => {
  ...
  try {
    ...
    res.send('Arquivo XML criado com sucesso!');
  } catch (error) {
    console.error('Erro ao acessar a página HTML:', error);
    res.status(500).json({ error: 'Erro ao processar os links fornecidos.' });
  }
});
...
const getDataFromHtml = (html) => {
  ...
  var productBrand = $('.po-brand td:last-child span').text().trim() || 'HEINZ'
